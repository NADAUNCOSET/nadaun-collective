...
export default function AiInnovationLabOverlay({isOpen,onClose,onBack,onContactClick,initialSection='overview'}:Props) {
  ...
  const jump=(id:string)=>{
    ...
    if(container&&target)container.scrollTo({top:target.getBoundingClientRect().top-container.getBoundingClientRect().top+container.scrollTop-80,behavior:'smooth'});
  };
  ...
  return <InsightsScrollContext.Provider value={scrollRef}>
    <motion.section ref={scrollRef} role="dialog" aria-modal="false" aria-label={initialSection==='signage'?'NADAUN Digital Signage':initialSection==='production'?'ALL IN ONE SOLUTION':'NADAUN Lab'} className="lab-page collective-editorial fixed inset-0 z-[105] overflow-y-auto overflow-x-hidden" initial={{y:'100%'}} animate={{y:0}} transition={{duration:.4,ease:[.16,1,.3,1]}}>
      ...
      <div className="lab-content">
        ...
        {initialSection!=='signage'&&<section id="lab-workspace" className="lab-section">
          <ScrollStory intro={<div className="lab-opening"><div><p className="lab-eyebrow">Production platform</p><h1><SceneLine visible>ALL IN ONE</SceneLine><SceneLine order={1}>SOLUTION.</SceneLine></h1><p className="lab-lead">나다운의 프리프로덕션 자동화 시스템.</p><a className="lab-link" href={WORKSPACE} target="_blank" rel="noopener noreferrer">워크스페이스 열기 <ArrowUpRight size={18}/></a><p className="lab-access">운영 워크스페이스 · 승인된 계정으로 접속</p></div><StoryMedia src="https://media.nadaun.co/allinone/projects/NIKE-OLIVE-BURGUNDY-12-SCENE-MAGAZINE-FILM/01_STILL/S01_HERO-26ec0115fb772ac7.webp" alt="올인원 솔루션의 Nike 콘셉트 프리뷰" caption="ALL IN ONE SOLUTION · NIKE 콘셉트 프리뷰"/></div>} impact={<div className="lab-statement"><p className="lab-eyebrow">Connected production</p><h2>기획의 판단을.<br/>촬영의 기준으로.</h2><p>온라인·오프라인 광고 제작에 필요한 기획 문서와 촬영 준비 정보를 한 프로젝트에서 연결합니다. 기획안, 스토리보드·콘티, 애니메틱, 샷리스트의 반복 입력과 문서 정리를 자동화해 준비 시간을 줄입니다. 연출과 제작의 판단은 팀이 맡고, 확정된 컷과 수정 이력은 후속 작업의 기준으로 관리합니다.</p></div>}/>
        ...
        {initialSection!=='production'&&<section id="lab-signage" className="lab-section">
          <ScrollStory intro={<div className="lab-opening"><div><p className="lab-eyebrow">Marketing · Digital signage</p><h1><SceneLine visible>사이니지 제작.</SceneLine><SceneLine order={1}>공간을 위한 미디어.</SceneLine></h1><p className="lab-lead">스마트 TV와 설치기사 1명. 간결한 현장 구축.</p></div><StoryMedia src="/hero/pepsi-festa.webp" alt="펩시 페스타의 무대 디스플레이와 브랜드 콘텐츠" caption="콘텐츠 제작 사례 · Pepsi Festa"/></div>} impact={<div className="lab-statement"><p className="lab-eyebrow">Platform · Network · Contents</p><h2>설치된 화면에.<br/>콘텐츠가 이어지도록.</h2><p>국내외 F&B·리테일·호텔과 브랜드 공간에 적용하는 사이니지 플랫폼을 제작·공급합니다. 스마트 TV와 현장 설치기사 1명을 기본 구성으로, 나다운이 설치와 네트워크를 관리합니다. 사진·영상 제작 플로우, 콘텐츠 편성과 배포를 온라인 운영 환경에 연결해 반복 업무를 자동화합니다.</p></div>}/>
          ...
          <ScrollReveal className="lab-signage-types"><span>F&B</span><span>Retail</span><span>Hospitality</span><span>Brand spaces</span></ScrollReveal>
