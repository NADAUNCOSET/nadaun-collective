...
export default function StarloginOverlay({onClose,onContactClick}:Props){
  const scrollRef=useRef<HTMLElement>(null),reduce=useReducedMotion();
  ...
  return <InsightsScrollContext.Provider value={scrollRef}>
    <motion.section ref={scrollRef} role="dialog" aria-label="STARLOGIN" className="lab-page starlogin-page collective-editorial fixed inset-0 z-[105] overflow-y-auto overflow-x-hidden" initial={reduce?false:{y:'100%'}} animate={{y:0}} transition={{duration:.4,ease:[.16,1,.3,1]}}>
